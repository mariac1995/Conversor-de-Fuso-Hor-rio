...
try:
    hora_escolhida = datetime.strptime(hora_texto, "%H:%M").time()
except ValueError:
    st.error("Formato inválido. Use HH:MM (ex: 09:30).")
    hora_escolhida = time(0, 0)
...
# CONVERSÃO
# -----------------------------
fuso_origem = pytz.timezone(locais[origem])
fuso_destino = pytz.timezone(locais[destino])

data_hora = datetime.combine(data_escolhida, hora_escolhida)

horario_origem = fuso_origem.localize(data_hora)
horario_destino = horario_origem.astimezone(fuso_destino)
...
diferenca = (
    horario_destino.utcoffset() - horario_origem.utcoffset()
).total_seconds() / 3600

st.info(f"Diferença de horário: {diferenca:+.0f} horas")
...
if diferenca > 0:
    texto = f"{destino} está {diferenca:.0f} horas à frente de {origem}."
elif diferenca < 0:
    texto = f"{destino} está {abs(diferenca):.0f} horas atrás de {origem}."
else:
    texto = f"{destino} tem o mesmo horário de {origem}."

st.info(texto)


# Mostra quadro comparativo
st.divider()
...
with col1:
    st.write(f"**{origem}**")
    st.write(f"Data: {horario_origem.strftime('%d/%m/%Y')}")
    st.write(f"Hora: {horario_origem.strftime('%H:%M')}")

with col2:
    st.write(f"**{destino}**")
    st.write(f"Data: {horario_destino.strftime('%d/%m/%Y')}")
    st.write(f"Hora: {horario_destino.strftime('%H:%M')}")